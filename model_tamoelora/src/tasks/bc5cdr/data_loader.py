...
from ..utils_sample import filter_sentence, sample_unlabelld
...
def get_bc5cdr_hf(
    ...
    dataset = load_dataset("ghadeermobasher/BC5CDR-Chemical-Disease", trust_remote_code=True)
...
class Bc5cdrDatasetLoader(DatasetLoader):
    ...
    def __init__(self, path_or_split: str, **kwargs) -> None:
        ...
        # filtered_examples = filter_sentence(examples, key='text')
        # sampled_data, label_counts = sample_unlabelld(filtered_examples, mention_key='entities', type_key='typed')
        
        for example in examples:
            idx = example['id']
            self.elements[idx] = example
