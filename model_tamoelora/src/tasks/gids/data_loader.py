...
from src.tasks.gids.guidelines import GUIDELINES
...
class GIDSDatasetLoader(DatasetLoader):
    """
    A `DatasetLoader` for the GIDS dataset.
    ...
    ENTITY_TO_CLASS_MAPPING = {
        "education degree": EducationalDegree,
    ...
    RELATION_TO_CLASS_MAPPING = {
        ...
        "education degree": HasDegree,
        "education institution": GraduatedFrom,
    }
...
class GIDSSampler(Sampler):
    """
    A data `Sampler` for the GIDS dataset.

    Args:
        dataset_loader (`GIDSSampler`):
    ...
    def __init__(
        self,
        dataset_loader: GIDSDatasetLoader,
        task: str = None,
        split: str = "train",
