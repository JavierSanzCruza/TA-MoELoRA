...
from src.tasks.scierc.guidelines import GUIDELINES
# from src.tasks.scierc.guidelines_gold import EXAMPLES
...
class SCIERCDatasetLoader(DatasetLoader):
    """
    A `DatasetLoader` for the SciERC dataset.
    ...
    def __init__(self, path: str, **kwargs) -> None:
        self.elements = {}
        def find_category(data, start, end):
            for item in data:
                if item[0] == start and item[1] == end:
                    return item[2]  
            return None  
        with open(path, "r") as in_f:
            lines = json.load(in_f)
            
        for i, line in enumerate(lines):
            line_relations = line['relations']
            sentence = line['text']
            key = str(i)
...
class SCIERCSampler(Sampler):
    """
    A data `Sampler` for the SciERC dataset.

    Args:
        dataset_loader (`SCIERCSampler`):
    ...
    def __init__(
        self,
        dataset_loader: SCIERCDatasetLoader,
        task: str = None,
        split: str = "train",
