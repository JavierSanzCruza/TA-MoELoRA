...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, task_ids=None):
        if self.config.pretraining_tp > 1:
            ...
            gate_out = self.gate_proj(x, task_ids)
            up_out = self.up_proj(x, task_ids)
            act_fn_out = self.act_fn(gate_out)
            down_proj = self.down_proj(act_fn_out * up_out, task_ids)
            #down_proj = self.down_proj(self.act_fn(self.gate_proj(x, task_ids)) * self.up_proj(x, task_ids), task_ids)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            q = self.q_proj(hidden_states, task_ids)
            k = self.k_proj(hidden_states, task_ids)
            v = self.v_proj(hidden_states, task_ids)
        ...
        if self.config.pretraining_tp > 1:
            ...
            attn_output = self.o_proj(attn_output, task_ids)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, task_ids)
        hidden_states = residual + hidden_states
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, output_attentions, None, task_ids)
