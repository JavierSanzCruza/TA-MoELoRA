...
from transformers import BatchEncoding, PreTrainedTokenizerBase, PreTrainedModel
...
def prepare_data(
    ...
    if task_id_mapping_path:
        if task_id_mapping_path == 'cut_schema':
            # Find the prompt length
            split_keyword = '# This is the text to analyze'
            prompt = example.split(split_keyword)[0] + split_keyword
            prompt = tokenizer(
                text=prompt,
                max_length=max_length,
                truncation=True,
            ...
            # Create the weight mask
            schema_weights = np.zeros(len(model_inputs["labels"]), dtype=np.float32)

            for i in range(len(prompt)):
                schema_weights[i] = 1.0
            
            text_weights = np.zeros(len(model_inputs["labels"]), dtype=np.float32)
            for i, (label, schema) in enumerate(zip(schema_weights, loss_weight_mask)):
                if not label and not schema:
                    text_weights[i] = 1.0
            model_inputs["schema_weights"] = schema_weights
            model_inputs["text_weights"] = text_weights
            model_inputs["task_ids"] = task_id

        else:
            with open(task_id_mapping_path, 'r') as f:
                task_id_mapping = json.load(f)
            mapped_task_id = get_task_mapping(task_id, task_id_mapping)
            assert mapped_task_id is not None, f"Task id {task_id} not found in task id mapping"
            model_inputs["task_ids"] = mapped_task_id
    else:
        model_inputs["task_ids"] = task_id
        #raise ValueError("Task tokenizer is not provided. Please provide a task tokenizer.")
    
    return model_inputs

def get_task_mapping(string, mapping):
    for key in mapping:
        if key in string:
            return mapping[key]
    return None
            ...
            for example in examples:
                tokenized_examples.append(
                    prepare_data(
                        ...
                        task_id_mapping_path=task_id_mapping_path,
                    )
                )
...
class CollieDataset(Dataset):
    ...
    def __init__(
        ...
        self.task_id_mapping_path = task_id_mapping_path
        ...
        if self.split == "train":
            if self.task_embedding_path:
                epoch_datasets = glob.glob(
                    os.path.join(
                        self.task_embedding_path, f"{self.dataset_name}.{self.task_name}.{self.split}.*.pkl"
                )
            )
            else:
                epoch_datasets = glob.glob(
            ...
            if epoch_datasets:
                ...
                self.epoch_datasets = epoch_datasets 
                print(self.epoch_datasets)
                logging.info(f"Found {len(self.epoch_datasets)} pre-computed epoch datasets.")
                
                self.generate_epoch_datasets()
                
                # dataset = self.epoch_datasets[self.current_dataset_key]
                ...
                # self.dataset_dict = self.compute_tokenized_examples(
                #     dataset_path=dataset,
                #     num_workers=num_workers,
                #     tokenizer=tokenizer,
                #     task_embedding_path=task_embedding_path,
                # )
                    # torch.save(self.dataset_dict[int(epoch)], dataset_cache_path)
                    # logging.info(f"Data cached for future use.")
        ...
        if len(self.dataset_dict) == 0:
            ...
            # self.current_dataset_key = self.dataset_keys[0]
            logging.info(f"Loaded {len(self.dataset_dict)} examples from {dataset_path}")
        # logging.info(f"Loaded {[len(x) for x in self.dataset_dict.values()]} examples from {dataset_path}")


    def generate_epoch_datasets(self):
        if self.current_dataset_key < len(self.epoch_datasets):
            ...
            self.dataset_dict = self.compute_tokenized_examples(
                dataset_path=dataset,
                num_workers=1, # use 1 worker to reduce memory usage
                tokenizer=self.tokenizer,
                task_id_mapping_path=self.task_id_mapping_path,
                )
            logging.info(f"Loaded {len(self.dataset_dict)} examples from {dataset}")
            
        else:
            logging.info(f"Finished loading all pre-computed epoch datasets.")
    ...
    def compute_tokenized_examples(
        ...
        if 'jsonl' in dataset_path:
            ...
            examples = [{'text':json.loads(example.strip())["text"], 'task_id':json.loads(example.strip())["task_id"]} for example in examples]
        else:
            with open(dataset_path, 'rb') as f:
                examples = pickle.load(f)
            examples = [{'text':example["text"], 'task_id':example["embeddings"]} for example in examples]    
        # sample dataset when exceeding max_examples
        if self.max_examples is not None and self.max_examples < len(examples):
            rng = random.Random(42)
            examples = rng.sample(examples, self.max_examples)
    ...
    def __getitem__(self, idx) -> List[BatchEncoding]:
        return self.dataset_dict[idx].copy()
    ...
    def rotate_split(self):
        ...
        # ]
        self.current_dataset_key = self.current_dataset_key + 1
        
        if len(self.epoch_datasets) > 1:
            self.generate_epoch_datasets()
            logging.info(
...
class DataCollatorForCoLLIE:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        task_ids = [feature["task_ids"] for feature in features] if "task_ids" in features[0].keys() else None
        ...
        if loss_weight_mask is not None:
            ...
            for feature in features:
                ...
                if isinstance(feature["loss_weight_mask"], list):
                    feature["token_attention_mask"] = (
                        feature["loss_weight_mask"] + pad_remainder
                        if padding_side == "right"
                        else pad_remainder + feature["loss_weight_mask"]
                    )
                    ...
                    feature["token_attention_mask"] = np.concatenate([feature["loss_weight_mask"], pad_remainder]).astype(
                        np.float32
                    )
                    ...
                    feature["token_attention_mask"] = np.concatenate([pad_remainder, feature["loss_weight_mask"]]).astype(
                        np.float32
                    )
        ...
        if task_ids is not None:
            # activate this when implementing task mapping
            # with open('configs/task_mapping.json', 'r') as f:
            #     task_mapping = json.load(f)
            if isinstance(task_ids[0], str):
                for feature in features:
                    if 'token_attention_mask' in feature:
                        feature['task_ids'] = feature["token_attention_mask"]
                        feature.pop("token_attention_mask")
                    else:
                        feature['task_ids'] = 100 # placeholder
            else:
                for feature in features:
                    if 'token_attention_mask' in feature:
                        feature.pop("token_attention_mask")
        
        if 'schema_weights' in features[0].keys():
            for feature in features:
                feature['task_ids'] = feature["schema_weights"]
                # logging.info(f"using schema weights")
                feature.pop("schema_weights")
                feature.pop("text_weights")
            # for feature in features:
            #     feature['task_ids'] = feature["text_weights"]
            #     logging.info(f"using schema weights")
            
                if isinstance(feature["task_ids"], list):
                    feature["task_ids"] = (
                        feature["task_ids"] + remainder
                        if padding_side == "right"
                        else remainder + feature["task_ids"]
                    )  
                elif padding_side == "right":
                    feature["task_ids"] = np.concatenate([feature["task_ids"], remainder]).astype(
                        np.float32
                    )
                        
                else:
                    feature["task_ids"] = np.concatenate([remainder, feature["task_ids"]]).astype(
                        np.float32
                    )
