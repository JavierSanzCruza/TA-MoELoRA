...
class EventScorer(Scorer):
    ...
    def __call__(self, reference: Any, predictions: Any) -> Dict[str, Dict[str, float]]:
        ...
        for ref, pre in zip(reference, predictions):
            ...
            for pre_event in pre:
                ...
                if pre_event in ref:
                    ...
                    e_tp += 1
                    a_tp += len(ref_event & pre_event) # number of arguments that are correct in one event
            ...
            for event in ref2:
                ...
                # add argument type analysis
                keys = {key for key, value in event.__dict__.items() if value}  # get all keys that are not empty
                for key in keys:
                    if key not in ['mention', '_allow_partial_match']:
                        if key not in argument_scores:
                            argument_scores[key] = {"tp": 0, "total_pos": 0, "total_pre": 0}
                        argument_scores[key]["total_pos"] += 1
            ...
            for event in pre2:
                ...
                for key in keys:
                    if key not in ['mention', '_allow_partial_match']:
                        if key not in argument_scores:
                            argument_scores[key] = {"tp": 0, "total_pos": 0, "total_pre": 0}
                        argument_scores[key]["total_pre"] += 1
                if event in ref2:
                    event_scores[label]["tp"] += 1
                    ref2_event = ref2.pop(ref2.index(event))

                    # add argument type analysis
                    correct_keys = {
                        key for key in ref2_event.__dict__.keys()
                        if ref2_event.__dict__[key] == event.__dict__[key] and ref2_event.__dict__[key]  
                    }
                    for key in correct_keys:
                        if key not in ['mention', '_allow_partial_match']:
                            argument_scores[key]["tp"] += 1
        ...
        for argument in argument_scores:
            argument_scores[argument]["precision"] = (
                argument_scores[argument]["tp"] / argument_scores[argument]["total_pre"]
                if argument_scores[argument]["total_pre"] > 0.0
                else 0.0
            )
            argument_scores[argument]["recall"] = (
                argument_scores[argument]["tp"] / argument_scores[argument]["total_pos"]
                if argument_scores[argument]["total_pos"] > 0.0
                else 0.0
            )
            argument_scores[argument]["f1-score"] = (
                2
                * argument_scores[argument]["precision"]
                * argument_scores[argument]["recall"]
                / (argument_scores[argument]["precision"] + argument_scores[argument]["recall"])
                if (argument_scores[argument]["precision"] + argument_scores[argument]["recall"]) > 0.0
                else 0.0
            )
