...
def load_model(
    ...
    task_embedding_model: Optional[str] = None,
    task_dim: Optional[int] = None,
    task_id_mapping_path: Optional[str] = None,
    turn_off_last_layer_expert: Optional[int] = None,
    ...
    if config.model_type in MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING_NAMES:
        ...
        if config.model_type == "llama" and use_flash_attention:
            if 'Llama-3' in config._name_or_path:
                from src.model.patch_models.modelling_llama3 import LlamaForCausalLM as LlamaForCausalLMl_Llama3

                load_fn = LlamaForCausalLMl_Llama3
                kwargs = {"attn_implementation": "flash_attention_2"}
                logging.warning("Loading the model with flash attention 2")
    ...
    # if bnb_config:
    #     # Set load_in_4bit and load_in_8bit to False if they exist
    #     if hasattr(bnb_config, 'load_in_4bit'):
    #         bnb_config.load_in_4bit = False
    #     if hasattr(bnb_config, 'load_in_8bit'):
    ...
    if use_lora:
        ...
        if lora_weights_name_or_path is None:
            ...
            if lora_type == 'lora':
                ...
                raise ValueError(f"LoRA type {lora_type} is not supported. Supported types are 'lora' and 'moelora'.")
            ...
            model = PeftModel.from_pretrained(model, lora_weights_name_or_path, turn_off_last_layer_expert=turn_off_last_layer_expert)
    ...
    if use_lora:
        ...
        if lora_type == 'moelora':
            ...
            for name, module in model.named_modules():
                if isinstance(module, LoraLayer):
                    if torch_dtype == torch.bfloat16:
                        logging.debug(f"Converting MoELoRA layer {name} to {torch_dtype}")
                        module = module.to(torch.bfloat16)
    ...
    if inference:
        ...
        pass
        # no need to merge LoRA layers for inference
        # if use_lora:
