...
class ResultLogger:
    ...
    def __init__(self, task_name: str, do_ttest: bool) -> None:
    ...
    def compute_metrics(self, scorer: Any) -> Dict[str, Dict[str, Dict[str, float]]]:
        ...
        if self.do_ttest:
            ttest_scores = [scorer(reference=self.golds[i], predictions=self.predictions[i]) for i in range(len(self.golds))]
            scores["ttest"] = ttest_scores
...
def evaluate(
    ...
    with tqdm(total=len(data_args.test_tasks), desc="Evaluating") as pbar:
        for task in data_args.test_tasks:
            ...
            task_logger = ResultLogger(task, data_args.ttest)
            task_module = None
            scorer = None
